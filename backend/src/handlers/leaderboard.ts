...
export const get = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    // total_time_ms é calculado a partir da soma de time_taken_ms nas respostas
    const result = await query(
      `SELECT
         ...
         qp.cpf,
         qp.total_score,
         COALESCE(SUM(pa.time_taken), 0) as total_time_ms,
         ...
         ROW_NUMBER() OVER (ORDER BY qp.total_score DESC, COALESCE(SUM(pa.time_taken), 0) ASC) as position
       FROM quiz_participants qp
       LEFT JOIN users u ON qp.user_id = u.id
       LEFT JOIN participant_answers pa ON pa.participant_id = qp.id
       WHERE qp.quiz_id = $1
       GROUP BY qp.id, qp.user_id, qp.cpf, qp.participant_name, u.name, qp.total_score
       ORDER BY qp.total_score DESC, total_time_ms ASC`,
      [quizId]
    );
